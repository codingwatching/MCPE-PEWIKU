...
GameRenderer::GameRenderer(Minecraft* mc)
	: mc(mc), renderDistance(0), _tick(0), _lastTickT(0), fovOffset(0), fovOffsetO(0), fov(1), oFov(1),
	  _setupCameraFov(0), zoom(1), zoom_x(0), zoom_y(0), cameraRoll(0), cameraRollO(0), pickDirection(1, 0, 0),

	  thirdDistance(4), thirdDistanceO(4), thirdRotation(0), thirdRotationO(0), thirdTilt(0), thirdTiltO(0),

	  fogBr(0), fogBrO(0),

	  fr(0), fg(0), fb(0), _rotX(0), _rotY(0), _rotXlast(0), _rotYlast(0), useScreenScissor(false) {
	saveMatrices();
...
void GameRenderer::renderLevel(float a) {
	...
	for (int i = 0; i < 2; i++) {
		...
		if (useScreenScissor) {
			...
			glScissor(screenScissorArea.x, screenScissorArea.y, screenScissorArea.w, screenScissorArea.h);
		}
...
void GameRenderer::tick(int nTick, int maxTick) {
	...
	float brr = mc->level->getBrightness(Mth::floor(mc->cameraTargetPlayer->x), Mth::floor(mc->cameraTargetPlayer->y),
...
void GameRenderer::setupGuiScreen(bool clearColorBuffer) {
	...
	GLbitfield clearBits = clearColorBuffer ? GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT : GL_DEPTH_BUFFER_BIT;
	glClear(clearBits);
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity2();
